...
export async function createIngredient(body: { name: string; unit?: string; stock?: number; min_stock?: number; cost?: number; supplier?: string; photo?: string | null }): Promise<Done<Ingredient>> {
  const user = me();
  ...
  const at = nowISO();
  const id = uuid();
  const clean = { name: body.name.trim(), unit: body.unit || "u", stock, min_stock: Number(body.min_stock || 0), cost: Number(body.cost || 0), supplier: body.supplier || "" };
